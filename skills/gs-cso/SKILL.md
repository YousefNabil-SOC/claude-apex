...
preamble-tier: 2
version: 2.0.0
description: "Chief Security Officer security audit - OWASP Top 10 plus STRIDE threat modeling. CARL TRIGGERS: security audit, OWASP audit, STRIDE, vulnerability check, security review, threat model, pen test mindset. SOURCE: garrytan/gstack/cso, integrated as gs-cso on 2026-05-29."
allowed-tools:
  - Bash
  - Read
  - Grep
...
> INTEGRATION NOTE: Extracted from gstack and renamed gs-cso to coexist with the host CARL / CAPABILITY-REGISTRY routing. Original gstack documentation follows. Sibling gstack skills referenced below by their original names map to gs-* when installed here (e.g. /review -> gs-review). gstack skills NOT installed in this environment (ship, qa, codex, office-hours, browse, freeze, guard, land-and-deploy) and gstack-only infra (browse binary, ~/.gstack GBrain, bun eval harness) are unavailable, so those references are informational only.
