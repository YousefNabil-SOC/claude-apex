...
preamble-tier: 3
version: 2.0.0
description: "YC Office Hours - six forcing questions that reframe a product idea before any code is written. CARL TRIGGERS: new product idea, explore an idea, ideate, before i build, scope this concept, is this worth building, brainstorm something new. SOURCE: garrytan/gstack/office-hours, integrated as gs-office-hours on 2026-05-29."
allowed-tools:
  - Bash
  - Read
  - Grep
...
> INTEGRATION NOTE: Extracted from gstack and renamed gs-office-hours (Phase 1B) to coexist with the host environment's CARL routing. Original gstack docs follow. Sibling gstack skills referenced below map to gs-* when installed (the 19-skill set); gstack-only infra (browse binary, gstack bin/ toolchain, ~/.gstack GBrain) is not installed here, so dependent steps degrade gracefully.
