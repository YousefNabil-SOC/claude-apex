...
8. **PostCompact Hook**: Check $HOME/.claude/hooks/post-compact-recovery.sh exists
9. **Sound Notification**: Check if $HOME/.claude/hooks/task-complete-sound.sh exists
10. **Peers Auto-Register**: Check if $HOME/.claude/hooks/peers-auto-register.sh exists
...
|---|--------|--------|---------|
| 1 | OMC Plugin | OK/FAIL | ... |
| ... | ... | ... | ... |

After the table:
- Count total OK vs FAIL
- If any system shows FAIL, suggest a specific fix
- Show environment totals: skills, agents, MCP servers, plugins
